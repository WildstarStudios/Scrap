...
class ListHandler(StatementHandler):
    keywords = ['list ']

    def can_handle(self, line):
        return line.strip().startswith('list ')

    def parse(self, lines, start_index):
        line = strip_comments(lines[start_index]).strip()
        m = re.match(r'^list\s+([a-zA-Z_]\w*)\s*(?:as\s+([a-zA-Z_]\w*))?\s*=\s*\[([^\]]*)\]$', line)
        if not m:
            raise SyntaxError("Expected: list name [as Type] = [elem1, elem2, ...]")
        name = m.group(1)
        type_part = m.group(2)
        ...
        if type_part:
            ...
            if elements:
                ...
                cpp_elem_type = 'int'
        cpp_vec_type = f'std::vector<{cpp_elem_type}>'
        return ('LIST', name, cpp_vec_type, elements), start_index + 1
    ...
    def generate(self, node, indent=''):
        ...
        return f'{indent}{vec_type} {name} = {init_list};'
